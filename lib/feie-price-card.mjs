...
export function buildFeiePriceCard({ product: rawProduct, settings: rawSettings, feedLines = 0 } = {}) {
  ...
  // Barcode is deliberately first so shelf staff can scan it immediately.
  if (barcode.text) {
    lines.push(`<C>${barcode.tag}</C>`);
    lines.push(`<C>${barcode.text}</C>`);
  }
  if (settings.storeName) lines.push(`<BOLD>${settings.storeName}</BOLD>`);
  // Use normal-size bold instead of <B>, because <B> doubles the product name size on Feie printers.
  if (settings.showNameZh) lines.push(`<BOLD>${nameZh}</BOLD>`);
