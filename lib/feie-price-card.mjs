...
export function buildFeiePriceCard({ product: rawProduct, settings: rawSettings } = {}) {
  ...
  if (settings.showSpec && product.spec) bodyLines.push(`規格：${product.spec}`);
  ...
  const barcodeMarkup = compactBarcode
    ...
    : barcode.text
      ? `<C>${barcode.tag}</C>`
      : '';
  // Barcode is printed first. Do not insert an extra BR around it; the barcode
  // command itself advances the printer, which keeps the physical whitespace
  // as compact as the verified previous top-barcode layout.
  const markup = barcodeMarkup ? `${barcodeMarkup}${bodyMarkup}` : bodyMarkup;
