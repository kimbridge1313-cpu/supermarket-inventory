...
export function buildFeiePriceCard({ product: rawProduct, settings: rawSettings, feedLines = 4 } = {}) {
  ...
  // Feie Code128 already prints its human-readable value under the bars.
  // Do not add a second barcode-number line.
  if (barcode.text) lines.push(`<C>${barcode.tag}</C>`);
  if (settings.storeName) lines.push(`<BOLD>${settings.storeName}</BOLD>`);
  // L = double height only. This keeps the Chinese name prominent without
  // the excessive width of B (double width + height).
  if (settings.showNameZh) lines.push(`<L><BOLD>${nameZh}</BOLD></L>`);
  ...
  // The cutter is physically below the print head. Feed paper after the last
  // content line so the price/content clears the cutter before cutting.
