...
async function downloadCsv(token) {
  let response = await fetch(`${DRIVE_API}/files/${FILE_ID}?alt=media`, { headers: { Authorization: `Bearer ${token}` } });
  if (!response.ok) {
    response = await fetch(`https://drive.usercontent.google.com/download?id=${FILE_ID}&export=download&confirm=t`);
  }
...
async function commitBatch(token, products, nextCursor, meta) {
  ...
  writes.push({
    update: {
      name: `${documentsRoot()}/importJobs/${JOB_ID}`,
      fields: fieldsFromObject({ ...meta, cursor: nextCursor, completed: nextCursor >= meta.importableRows, updatedAt: importedAt })
    }
  });
  await fsRequest(token, `https://firestore.googleapis.com/v1/projects/${projectId()}/databases/(default)/documents:commit`, {
    method: "POST",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const job = reset ? null : await readJob(token);
    let cursor = reset ? 0 : Math.max(0, Number(job?.cursor || 0));
    let commits = 0;
    const callLimit = Math.min(importable.length, cursor + MAX_PER_CALL);

    while (cursor < callLimit && Date.now() - started < MAX_RUN_MS) {
      const nextCursor = Math.min(callLimit, cursor + BATCH_SIZE);
      const batch = importable.slice(cursor, nextCursor);
      await commitBatch(token, batch, nextCursor, { ...meta, startedAt: job?.startedAt || meta.startedAt, status: nextCursor >= importable.length ? "completed" : "running" });
      cursor = nextCursor;
      commits += 1;
    }
    ...
    return res.status(200).json({
      ok: true,
      cursor,
      importedThisCall: commits * BATCH_SIZE - Math.max(0, commits * BATCH_SIZE - (cursor - Math.max(0, Number(job?.cursor || 0)))),
      commits,
      completed,
      productCount,
    ...
    console.error("temporary CSV import failed", error);
    return res.status(500).json({ ok: false, message: error instanceof Error ? error.message : String(error), elapsedMs: Date.now() - started });
  }
}
